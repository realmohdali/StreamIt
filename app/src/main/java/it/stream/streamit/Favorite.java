...
public class Favorite extends AppCompatActivity implements SwipeToRemove.SwipeToRemoveListener, RemoveQueueItem.SwipeToRemoveListener {
    ...
    private void loadFav() {
        FavoriteManagement favoriteManagement = new FavoriteManagement(getApplicationContext(), db);
        mList = favoriteManagement.showFavList();
        if (mList.size() > 0) {
    ...
    private BroadcastReceiver listChanged = new BroadcastReceiver() {
        @Override
    ...
    private void registerListChanged() {
        IntentFilter filter = new IntentFilter(List_Changed);
        registerReceiver(listChanged, filter);
    }
